...
bool container::parse(file_metadata* md, size_t max_num_rows, size_t first_row)
{
  ...
  while (m_cur + 18 < m_end && total_object_count < max_num_rows) {
    ...
    md->all_block_list.emplace_back(
      m_cur - m_base, block_size, total_object_count, first_row, object_count);
    // Break on terminal conditions.  We keep these as separate lines instead of
